...
- `app/services/image_generation.py` содержит `ImageGenerator`, который обращается к модели `gpt-image-1`, применяет ретраи и возвращает бинарные данные изображения, и `ImagePipeline`, сочетающий генератор с Google Drive для получения финальной ссылки. Качество изображения задаётся параметром `IMAGE_QUALITY`.
