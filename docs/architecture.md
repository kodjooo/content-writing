...
Основной модуль `app/main.py` поддерживает два режима: однократный запуск (`run_once`) и планировщик, который с помощью `pytz` вычисляет ближайшее время запуска и ежедневно в заданный час выполняет обработку без использования cron. Дополнительно флаг `RUN_ON_START` позволяет выполнить один запуск сразу после старта контейнера перед переходом к циклу планировщика.
