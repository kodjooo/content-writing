...
- Уточнить структуру `.env`, собрать обязательные переменные (ключ OpenAI, путь к `service_account.json`, идентификатор таблицы, параметры обработки, ключ хостинга изображений) и добавить пример в репозиторий (`.env.example`).
