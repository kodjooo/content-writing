...
- Актуализировать тестовые сценарии и покрытие документируемых контрактов (выполнено).
...
- Актуализировать README по фактическому поведению рантайма и переменных окружения (выполнено).

## Этап 10. Запуск при старте перед расписанием (выполнено)
- Добавить флаг `RUN_ON_START` для выполнения единичного запуска при старте контейнера до входа в планировщик.
- Обновить документацию, конфиги окружения и тесты под новый режим.
- Провести полную синхронизацию `docs/requirements.md`, `docs/architecture.md`, `.env` и `.env.example` с текущей реализацией (выполнено).
