...
class ImageGenerator:
    ...
    def generate(self, prompt: str, size: Optional[str] = None) -> bytes:
        ...
        def _call() -> bytes:
            ...
            if not payload and hasattr(first_item, "image"):
                payload = getattr(first_item.image, "b64_json", None)
            if not payload and isinstance(first_item, dict):
                payload = first_item.get("b64_json")
            if not payload:
